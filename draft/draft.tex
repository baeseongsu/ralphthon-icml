...
\icmltitlerunning{Building Reliable Review Agents from Multiple Human Reviews}

\begin{document}

\twocolumn[
  \icmltitle{Building Reliable Review Agents from\\
    Multiple Human Peer Reviews}
...
We group public ICML 2026 OpenReview records by paper forum identifier. The accepted-paper collection contains 6,341 papers and 24,378 official reviews. We sample 600 papers (9.5\%) using largest-remainder allocation over primary areas and, within each area, quartiles of mean reviewer overall recommendation (seed 42). The sample covers 81 of 84 areas; its mean score is 4.144 versus 4.147 in the population, and its spotlight share is 7.7\% versus 8.5\%. We generated consolidated labels for 175 sampled papers (166 regular and 9 spotlight). \Cref{fig:pipeline} summarizes the workflow.

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.66\textwidth]{../psudo_labeling/figure/pipeline/pseudo_labeling_pipeline.png}
  \caption{Pseudo-labeling workflow. A stratified ICML review sample is processed one paper at a time. Reviewer evidence is prioritized and deduplicated before producing a single structured review whose scores remain within the human-review range. Reviewer-priority metadata is retained for analysis only; the pseudo-review is the downstream supervision target.}
  \label{fig:pipeline}
\end{figure*}
...
We use the version-2 consolidation prompt with Claude Opus 4.8, assigning one labeling agent to each paper and running up to ten agents in parallel. Each input contains the paper title, abstract, primary area, initial reviews, rubric scores, and meta-review; explicit rebuttal and post-rebuttal \texttt{final\_justification} fields are excluded. The prompt first asks the model to classify source reviews as high, medium, or low priority using specificity, cross-review corroboration, and agreement with the meta-review. It then instructs the model to write one first-person ICML review, merge repeated observations and questions, exclude low-quality or unsupported criticisms, and avoid any mention of other reviewers, the meta-review, rebuttal, score changes, or the decision. The complete prompt used for label construction is included in the repository at \texttt{psudo\_labeling/docs/pseudo\_labeling\_prompt.md}.
...
The deployment artifact freezes P2 at \texttt{skills/auto-research/assets/review-optimization/smoke-prompt-calibration-v3.md} and pairs it with \texttt{generated-review.schema.json}. Under \texttt{skills/auto-research/scripts/}, \texttt{review\_prompt\_codex.py} implements inference; adjacent \texttt{review\_prompt\_*} scripts implement dataset freezing, batching, scoring, reflection, selection, and privacy-aware tracking. The deployment manifest records prompt and schema hashes. P3 and P4 remain opt-in experimental prompts.
